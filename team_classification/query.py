...
frame_box_list = []
color_space = 'rgb'
query_files = ['output_0051_clip2.jpg', 'output_0052_clip15.jpg', 'output_0047_clip3.jpg']
...
for i,f in enumerate(os.listdir(img_dir)):
    ...
    img = imread(os.path.join(img_dir, f), pilmode='RGB')
    print(f)
    frame = hyphen_split(f.split('.')[0]).split('_')[1]
    box = hyphen_split(f.split('.')[0]).split('_')[2][4:]
